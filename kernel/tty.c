...
int
tty_write (tty *tty, char *buf, int length)
{
    for (size_t i = 0; i < length; i++)
    {
        ...
        {
            /* We'll clear the buffer, most of the input was unwanted anyway if we got that far. */
...
int
tty_read (tty *tty, char *buf, int length)
{
    int bytes_read = 0;
...
void
tty_handle_key (tty *tty, char key)
{
    int next_head = (tty->input_head + 1) % 1024;
    if (next_head == tty->input_tail)
    {
